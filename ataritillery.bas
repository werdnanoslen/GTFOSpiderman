...
main
 if sc1=$00 && sc2=$00 && sc3>24 then count=10 : goto win  
 ...
 rem adjust power
 if joy0up then p0power=p0power+0.1
 if joy0down then p0power=p0power-0.1
 ...
 rem play ball
 if joy0fire && ballflies=0 then ballflies=1 : random=rand
 if collision(ball,playfield) then ballflies=0 : goto main
 if collision(ball,player1) && lethal=1 && ballflies=1 then ballflies=0 : goto hit
 if collision(ball,player1) && lethal=0 && ballflies=1 then ballflies=0 : goto catch
